...
class ZMQPortAllocation:
    """ZMQ port allocation for a benchmark run"""

    def __init__(
        self,
        event_bus_frontend: int,
        event_bus_backend: int,
        dataset_frontend: int,
        dataset_backend: int,
        raw_inference_frontend: int,
        raw_inference_backend: int,
        records_port: int,
        credit_drop_port: int,
        credit_return_port: int,
    ):
        self.event_bus_frontend = event_bus_frontend
        self.event_bus_backend = event_bus_backend
        self.dataset_frontend = dataset_frontend
        self.dataset_backend = dataset_backend
        self.raw_inference_frontend = raw_inference_frontend
        self.raw_inference_backend = raw_inference_backend
        self.records_port = records_port
        self.credit_drop_port = credit_drop_port
        self.credit_return_port = credit_return_port
...
class PortAllocator:
    ...
    def _find_available_block(self) -> Optional[int]:
        ...
        for base_port in range(
            self.PORT_POOL_START, self.PORT_POOL_END - self.PORTS_PER_RUN
        ):
